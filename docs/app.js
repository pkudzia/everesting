...
function renderTotals(c) {
  const items = [
    ['Target', num(c.target_m), 'm'],
    ['Planned climbing', num(c.totals.gain_m), 'm'],
    ['Laps', String(c.totals.laps), ''],
    ['Uphill distance', num(c.totals.distance_km), 'km'],
  ];
  const dl = $('#totals');
...
function renderPlan(c) {
  $('#plan-note').textContent =
    `Longer, gentler variations while fresh; the short steep lines saved for the end. ` +
    `The running total crosses ${num(c.target_m)} m on lap ${c.totals.crossed_on_lap}, ` +
    `and finishing all ${c.totals.laps} laps banks ${num(c.totals.gain_m)} m — ` +
    `a ${num(c.totals.gain_m - c.target_m)} m buffer in case the watch disagrees with the map.`;
...
function profile(v) {
  ...
  function draw() {
    ...
    dot = node('circle', { r: 3.5, fill: '#fff', opacity: 0, cx: 0, cy: 0 });
